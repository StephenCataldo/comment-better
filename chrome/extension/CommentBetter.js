...
*/
/* commented out for CommentBetter for now.
 */ 
...
/** Ok, look for these facebook "nodes" and count them
 * so we don't process it twice **/
var config = {};
config.fb_post_search = '[role="article"] ._1dwg'; // facebook posts
var fb_post_count = 0;


/* Not jquery, perhaps use: document.querySelectorAll */

/** This shit code runs over and over on your facebook page,
 * seeking out the comment sections and adding our stuff there.
 * hackathon! **/


  /* this code runs over and over on the same stuff, but for hackathon.
 * See how it's dealth with here:
 * [StephenCataldo:~/Downloads/Official Media Bias_Fact Check Extension] cataldo% vim src/inject/facebook_filter.js   */
  process("first");
  var keeptrying = 40;
...
//      observer.disconnect(); // something went wrong.
    });
  });
...
function process(name) {
    ...
    var posts = document.querySelectorAll(config.fb_post_search);
     console.log("posts from " + config.fb_post_search + " is: ");
     console.log(posts);
     console.log("posts length is " + posts.length + ", and the count is " + fb_post_count);
    let start = fb_post_count;  
    console.log("Go from " + start + " to " + posts.length);
    for (let i = start; i < posts.length; i++) { 
      // Problem: some of these can be unrendered. Which means
      // it's too early! 
      fb_post_count++;
      // what is posts[i]
      console.log("Post #" + i);
      ...
      // for each NEW post, look for comments section
      let comments = posts[i].getElementsByClassName('.UFICommentAttachmentButtons');
      console.log("Is there a comment box?");
      console.log(comments);
      for (let j = 0; j < comments.length; j++) {
          console.log("Trying for red");
          comments[i].style.backgroundColor = "red";
      }
        
    } 

/* Seems to sometimes work on one post=node
  console.log("Fire up editing facebook comment box: " + name);
  $('.highlight').css({
    'background': 'yellow',
    'border': '1px solid blue'
    });
  //$('.UFICommentAttachmentButtons').append('***<img src="./images/attachButton.png">');
